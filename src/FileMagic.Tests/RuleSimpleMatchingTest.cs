...
namespace ldy985.FileMagic.Tests
{
    public class RuleSimpleMatchingTest : IDisposable
    {
        public RuleSimpleMatchingTest()
        {
            ...
            _simpleSignatureMatcher = new FileMagic(NullLogger<FileMagic>.Instance, ruleProvider, new TrieSignatureMatcher(NullLogger<TrieSignatureMatcher>.Instance, ruleProvider), null, Microsoft.Extensions.Options.Options.Create(fileMagicConfig));
        }
        ...
        public void LongRuleMatchCase()
        {
            using (MemoryStream ms = new MemoryStream(new byte[] { 0x0, 0x0, 0x0, 0x0, 0x0, 0x1, 0x2 }))
                Assert.True(_simpleSignatureMatcher.StreamMatches<TestRule2>(ms, out IResult result));
        }
        ...
        public void LongRuleNoMatchCase()
        {
            using (MemoryStream ms = new MemoryStream(new byte[] { 0x0 }))
                Assert.False(_simpleSignatureMatcher.StreamMatches<TestRule2>(ms, out IResult result));
        }
        ...
        public void LongSteamCase()
        {
            using (MemoryStream ms = new MemoryStream(new byte[] { 0x0, 0x0, 0x0, 0x0, 0x0 }))
                Assert.False(_simpleSignatureMatcher.StreamMatches<TestRule>(ms, out IResult result));
        }
        ...
        public void MatchCase()
        {
            using (MemoryStream ms = new MemoryStream(new byte[] { 0x0, 0x1, 0x2 }))
                Assert.True(_simpleSignatureMatcher.StreamMatches<TestRule>(ms, out IResult result));
        }
        ...
        public void WildcardRuleCase()
        {
            using (MemoryStream ms = new MemoryStream(new byte[] { 0x0, 1, 2, 7 }))
                Assert.True(_simpleSignatureMatcher.StreamMatches<TestRule3>(ms, out IResult result));
        }
    }
}