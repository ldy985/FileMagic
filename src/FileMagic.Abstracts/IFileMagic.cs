...
namespace ldy985.FileMagic.Abstracts
{
    public interface IFileMagic
    {
        ...
        bool IdentifyStream(BinaryReader binaryReader, [NotNullWhen(true)] out IResult? result, in IMetaData metaData);
        ...
        /// <param name="result">The result</param>
        /// <returns>True if we found a match.</returns>
        bool IdentifyStream(BinaryReader binaryReader, [NotNullWhen(true)] out IResult? result);
        ...
        bool StreamMatches<T>(BinaryReader binaryReader, [NotNullWhen(true)] out IResult? result) where T : IRule;
    }
}